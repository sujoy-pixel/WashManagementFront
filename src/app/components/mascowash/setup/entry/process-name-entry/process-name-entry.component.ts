...
import { ProcessNameEntryService } from '../../../services/Setup/process-name-entry.service';
...
export class ProcessNameEntryComponent {
  ...
  constructor(
    private service: ProcessNameEntryService,
  ...
  async ngOnInit() {
    this.LoadCompanyBankInfoCompanyList();
  ...
  //////////////////////////Load Dropdown Start//////////////////////
  LoadCompanyBankInfoCompanyList() {
    this.CompanyList = [];
    this.service.GetCompanyBankInfoUnitNameAndAddress().subscribe(
      (data: any[]) => {
        this.CompanyList.push({ label: '--- Select ---', value: null });
        for (var i = 0; i < data.length; i++) {
          this.CompanyList.push({
            label: data[i].displayName,
            value: data[i].id,
            companyAdd: data[i].option1,
          });
        }
      },
      (error) => {}
